...
void AF_Stepper::setSpeed(uint16_t rpm) {
  ...
  //us/min * ( rev/steps * min / rev ) = us/min * ( min/steps ) = us/steps
  //steppingcounter = 0;
}

void AF_Stepper::setSpeedMicroseconds(uint32_t us) {
  usperstep = us;
  //steppingcounter = 0;
}
...
void AF_Stepper::step(uint16_t steps, uint8_t dir,  uint8_t style) {
  ...
  while (steps--) {
    if(Serial.available() > 0) {steps = 0;}
    if(breakStepping){steps = 0; breakStepping = false;}
